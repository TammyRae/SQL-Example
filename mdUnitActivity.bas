Attribute VB_Name = "mdUnitActivity"
'******************************************
...
Public Const EXTERNAL_COMM_COLUMN As Integer = 21



Rem ****************************************************************************
Rem *
Rem * Function Name:    GenerateUnitActivityReport
...
Rem ****************************************************************************
Sub GenerateUnitActivityReport(acUserName As String, acPassword As String, _
    ...
    Dim acCreatedTime As String, acExternalComm As String
    ...
    If Not (adoUnitList.BOF And adoUnitList.EOF) Then
        ...
        If iTotalRows > 0 Then
            ...
            Do While Not adoUnitList.EOF
                ...
                '* Get the last field report for this crew (external communication field)
                If excelWorksheet.Name = "Priority" Then
                    acSQLText = "select er.udf7"
                    acSQLText = acSQLText + " from r01_job_report r1, elec_spec_report er"
                    acSQLText = acSQLText + " where r1.isr_no = '" + acIsrNo + "'"
                    acSQLText = acSQLText + " and r1.unit = '" + Trim(acUnitNo) + "'"
                    acSQLText = acSQLText + " and r1.r01_report_no = er.r01_report_no"
                    acSQLText = acSQLText + " and r1.r01_create_date || r1.r01_create_time ="
                    acSQLText = acSQLText + " (select max(r01_job_report.r01_create_date || r01_job_report.r01_create_time)"
                    acSQLText = acSQLText + " from r01_job_report where r01_job_report.isr_no = r1.isr_no and r01_job_report.unit = r1.unit)"
                    adoFieldReport.Open acSQLText, adoConnection, adOpenStatic, adLockReadOnly, adCmdText
                    If Not (adoFieldReport.BOF And adoFieldReport.EOF) Then
                        If Not IsNull(adoFieldReport.Fields("UDF7")) Then
                            acExternalComm = adoFieldReport.Fields("UDF7")
                            excelWorksheet.Cells(iRowPosition, EXTERNAL_COMM_COLUMN) = acExternalComm
                        End If
                    End If
                    adoFieldReport.Close
                End If
                ...
                If frmMain.lstCategory.ListIndex = 0 Then
                    If Not adoUnitList.EOF Then
                        ...
                        End If
                    Else
                        'Write a row for the total crew and enroute-clear total
                        excelWorksheet.Cells(iRowPosition, 2) = iCrewTotal
                        excelWorksheet.Cells(iRowPosition, ENROUTE_TO_CLEARED_COLUMN) = "=SUM(R[-" + Trim(Str(iRowPosition - iStartRow)) + "]C:R[-1]C)/60"
                        excelWorksheet.Cells(iRowPosition, ENROUTE_TO_CLEARED_COLUMN).Select
                        excelExcelApp.Selection.NumberFormat = "0.00"
                        'Write the total greater than 15 for created-assigned
                        If excelWorksheet.Name = "Priority" Then
                            excelWorksheet.Cells(iRowPosition, CREATE_TO_ASSIGN_COLUMN) = "=COUNTIF(R[-" + Trim(Str(iRowPosition - iStartRow)) + "]C:R[-1]C,"">15"")"
...
Private Sub CreateWorksheetHeader(excelWorksheet As Excel.Worksheet, excelWorkBook As Excel.Workbook, excelApplication As Excel.Application)
    ...
    If excelWorksheet.Name = "Priority" Then
        excelWorksheet.Cells(1, EXTERNAL_COMM_COLUMN) = "External Comm"
        excelWorksheet.Columns(EXTERNAL_COMM_COLUMN).Select
    End If
