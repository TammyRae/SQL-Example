/* CUFOR144RCT: Commercial Water Field Activities Completed
	Reprot Title: Completed Commercial Water Reclaim FA's
...
inspection_result,
dep_acknowledgement,
cross_connect_inspect,
--reclaim_test_metrics,

 case substr(RECLAIMED_METER1,1,1)
        when  'S' then 'SAT'
        when 'U' then 'UNSAT'
        when 'N' then 'N/A'
        else 'N/A'
  end RECLAIMED_METER1,
    case substr(POTABLE_METER1,1,1)
        when  'S' then 'SAT'
        when 'U' then 'UNSAT'
        when 'N' then 'N/A'
        else 'N/A'
  end POTABLE_METER1,
      case substr(BACKFLOW_PREVENTER1,1,1)
        when  'S' then 'SAT'
        when 'U' then 'UNSAT'
        when 'N' then 'N/A'
        else 'N/A'
  end BACKFLOW_PREVENTER1,
  
      case substr(CONNECTIONS1,1,1)
        when  'S' then 'SAT'
        when 'U' then 'UNSAT'
        when 'N' then 'N/A'
        else 'N/A'
  end CONNECTIONS1,
   case substr(ADVISORY_SIGNS1,1,1)
        when  'S' then 'SAT'
        when 'U' then 'UNSAT'
        when 'N' then 'N/A'
        else 'N/A'
  end ADVISORY_SIGNS1,
      case substr(LEAKS1,1,1)
        when  'S' then 'SAT'
        when 'U' then 'UNSAT'
        when 'N' then 'N/A'
        else 'N/A'
  end LEAKS1,
      case substr(OVER_SPRAY1,1,1)
        when  'S' then 'SAT'
        when 'U' then 'UNSAT'
        when 'N' then 'N/A'
        else 'N/A'
  end OVER_SPRAY1,
      case substr(RUN_OFF1,1,1)
        when  'S' then 'SAT'
        when 'U' then 'UNSAT'
        when 'N' then 'N/A'
        else 'N/A'
  end RUN_OFF1,
      case substr(PONDING1,1,1)
        when  'S' then 'SAT'
        when 'U' then 'UNSAT'
        when 'N' then 'N/A'
        else 'N/A'
  ...
  NULL PAPER_WORK1, 
      case substr(POOL1,1,1)
        when  'S' then 'SAT'
        when 'U' then 'UNSAT'
        when 'N' then 'N/A'
        else 'N/A'
  end POOL1,
  NULL SEVEN_DAY_COMPLIANCE1,
      case substr(USES1,13,1)
        when  'S' then 'SAT'
        when 'U' then 'UNSAT'
        when 'N' then 'N/A'
        else 'N/A'
...
JEACUST.J_ISR_UDF_VAL_F(a.ISR_NO,
	...
	a3.meter_size,
	(SELECT digits_left FROM jea_register WHERE isr_no = a.isr_no AND ROWNUM < 2) AS meter_digits_left,
	a3.type meter_type,
  ...
  (SELECT y.badge_number FROM jea_item y WHERE y.isr_no = a.isr_no AND y.type_code = 'RID' AND ROWNUM < 2) om_register,
...
JEACUST.J_ISR_UDF_VAL_F(a.ISR_NO,
        ...
        a.P_FILTER1,
        '') inspection_result,
	NULL dep_acknowledgement,
JEACUST.J_ISR_UDF_VAL_F(a.ISR_NO,
        'Cross Connection Inspected',
        a.AGENCY_CODE, 
        a.INIT_SERVICE_CODE, 
        a.P_FILTER1,
        '') cross_connect_inspect,
	--b.test_result23 reclaim_test_metrics,
JEACUST.J_ISR_UDF_VAL_F(a.ISR_NO,
        'Reclaimed Meter',
        a.AGENCY_CODE, 
        a.INIT_SERVICE_CODE, 
...
WHERE a.isr_no = a3.isr_no (+)
  ...
  AND a.isr_no = u.isr_no (+)
	...
	AND a.p_filter1 = 'MTR-RW'
  and a.init_service_code not like '\_\_%' escape '\'

	AND (u.udf15 in ('RECCOM','RECRES','RECLAIM') 
		AND  DECODE(u.udf33,'5/8',.625,'5/8X3/4',.688,'3/4',.75,'1',1,'1 1/2',1.5,'2',2,'3',3,'4',4,'6',6,'8',8,
					'10',10,'12',12,'20',20,'NONE',0,'UNMTRED',0,0) >= 2)
...
JEACUST.J_ISR_UDF_VAL_F(a.ISR_NO,
	...
	a3.manufacturer_code meter_manu,
	a3.meter_size,
	(SELECT digits_left FROM his_jea_register WHERE isr_no = a.isr_no AND ROWNUM < 2) AS meter_digits_left,
	a3.type meter_type,
	a3.location_details meter_loc_details,
  JEACUST.J_ISR_UDF_VAL_F(a.ISR_NO,
        'Read Today A',
  ...
  (SELECT y.badge_number FROM his_jea_item y WHERE y.isr_no = a.isr_no AND y.type_code = 'RID' AND ROWNUM < 2) om_register,
